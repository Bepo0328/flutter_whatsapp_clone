...
class AuthRepository {
  ...
  void saveUserDataToFirebase({
    ...
    try {
      ...
      String photoUrl = Strings.photoUrl;
      ...
      await firestore.collection(Strings.user).doc(uid).set(user.toMap());
  ...
  Stream<UserModel> userData(String userId) {
    return firestore.collection(Strings.user).doc(userId).snapshots().map(
  ...
  void setUserState(bool isOnline) async {
    await firestore.collection(Strings.user).doc(auth.currentUser!.uid).update({
      'isOnline': isOnline,
    });
  }
}
